...
def run_producer(server = {"server": None, "port": None}, shared_id = None):
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        s_lon_0 = -179.9583  # -179.9958
        s_lat_0 = +83.95833588  # +83.99578094
        s_resolution = 5 / 60.0  # 30 / 3600.0

        for lat_100 in range(int(14.75*100), int(3.25*100 - 1) - 1):
            lat = lat_100 / 100.

            print(lat,)
            #if lon < int(config["start-row"]):
            #    continue
            #elif int(config["end-row"]) > 0 and srow > int(config["end-row"]):
            #    break

            for lon_100 in range(int(1.75*100), int(16.25*100 + 1)):
                lon = lon_100 / 100.
                print(lon,)
